// 执行器放到执行器内部
// 作用执行其
import { IAnyObject, IRenderConfig, IRenderConfigDependOn } from '@xigefish/d-render-shared'
  ...
  execute (type: string, logKey: string, ...args: [effects: IEffects, executeChangeValueEffect: boolean]) {
    ...
    if (args[0].length > 0) {
      if (!condition || (condition && condition(...args))) {
        ...
        console.log(`%c[@xigefish/d-render]%c执行%c${logKey}.${type}`, 'color: #e6a23c', 'color: normal', 'color: #67c23a')
      } else {
        console.log(`%c[@xigefish/d-render]%c不符合%c${logKey}.${type}%c执行条件`, 'color: #e6a23c', 'color: normal', 'color: red', 'color: normal', args[0])
      }
    }
  }
