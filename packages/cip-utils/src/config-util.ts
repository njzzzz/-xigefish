...
type TChangeValue = (
  values: IAnyObject,
  outValues: IAnyObject
) => { value: any; otherValue: any } | void;
type TChangeValueByOld = (
  { key, oldValue }: { key: string; oldValue: any },
  values: IAnyObject,
  outValues: IAnyObject
) => { value: any; otherValue: any } | void;
...
type TFormConfigUnion = IRenderConfig &
  IBaseFormRenderConfig &
  IFormRenderConfig;
type TSearchConfigUnion = IRenderConfig &
  IBaseFormRenderConfig &
  ISearchRenderConfig;
...
type TConfigListToMap =
  | ((configMap: IConfig<TFormConfigUnion>[]) => IFormFieldConfig)
  | ((configMap: IConfig<TTableConfigUnion>[]) => ITableFieldConfig)
  | ((configMap: IConfig<TSearchConfigUnion>[]) => ISearchFieldConfig);

export const configListToMap: TConfigListToMap = (configList) => {
  const result = {};
